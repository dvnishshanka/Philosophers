...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/17 11:26:38 by dnishsha          #+#    #+#             */
/*   Updated: 2024/01/17 12:15:01 by dnishsha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	has_phil_died(t_philo *philo)
{
	...
	if (get_int(&philo->mtx_philo, &philo->is_full) == 1)
		return (0);
	start_simulation_time = get_long(&philo->table->mtx_table,
			&philo->table->start_simulation);
	time_elapsed_from_start = get_timestamp(MILLISEC) - start_simulation_time;
	if (time_elapsed_from_start < get_long(&philo->table->mtx_table,
	...
	if (start_simulation_time != last_meal_time && time_elapsed_after_last_meal
		<= get_long(&philo->table->mtx_table, &philo->table->time_to_eat))
		return (0);
	if (time_elapsed_after_last_meal > get_long(&philo->table->mtx_table,
			&philo->table->time_to_die))
		return (1);
	return (0);
}
